...
router.post("/", authorization, async (req, res)=>{
    try {
        ...
        const newAppointment = await pool.query("INSERT INTO appointments (treatment_id, patient_id, appointment_date, appt_start_time, appointment_type, status) VALUES ($1, $2, $3, $4, $5, $6)RETURNING *", [
            treatmentid,
            patient_id,
            date,
            time,
        ...
        console.log(newAppointment.rows[0].appointment_id);
        const appointment_id = newAppointment.rows[0].appointment_id;

        //console.log(response);
        res.json({appointment_id});
    } catch (error) {
